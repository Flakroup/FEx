...
{
    public static bool IsRunning(this Task task) => task is not null
    ...
    public static bool IsFinished(this Task task) => task is not null
    ...
    public static bool IsFailed(this Task task) =>
        task is not null && task.Status is TaskStatus.Canceled or TaskStatus.Faulted;
    ...
    public static async Task<TRes[]>
        WhenAllAsync<T, TRes>(this IEnumerable<T> values, Func<T, Task<TRes>> asyncAction) =>
        await Task.WhenAll(values.Select(asyncAction));
    ...
    public static async Task RunFuncWithWhenAllAsync<T>(this IEnumerable<T> values,
                                                        Func<T, Action> asyncAction,
                                                        bool immediateStart = false)
    {
        await WhenAllAsync(immediateStart, values.Select(x => Task.Run(() => asyncAction(x))));
    }
    ...
    public static async Task<TRes[]> RunFuncTaskWithWhenAllAsync<T, TRes>(
        ...
        bool immediateStart = false)
    {
        return await WhenAllAsync(immediateStart,
            values.Select(x => Task.Run(() => asyncAction(x), cancellationToken)));
    }
    ...
    private static T[] Start<T>(IEnumerable<T> tasks) => tasks.ToArray();
