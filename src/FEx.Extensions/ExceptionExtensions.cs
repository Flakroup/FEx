...
{
    private static readonly Func<Exception, StackTrace, Exception> SetStackTraceFunc = new Func<Func<Exception, StackTrace, Exception>>(() =>
    {
        ...
        object normalTraceFormat = Enum.GetValues(traceFormatType)
            .GetValue(0);
        ...
        return Expression.Lambda<Func<Exception, StackTrace, Exception>>(Expression.Block(assign, target), target, stack)
            .Compile();
    })();
