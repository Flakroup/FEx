...
public class ConcurrentHashSet<T> : HashSet<T>
{
    ...
    public ConcurrentHashSet(IEnumerable<T> collection) : base(collection)
    {
    }
    ...
    public ConcurrentHashSet(IEnumerable<T> collection, IEqualityComparer<T> comparer) : base(collection, comparer)
    {
    }

    protected ConcurrentHashSet(SerializationInfo info, StreamingContext context) : base(info, context)
    {
    }

    public new IEnumerator<T> GetEnumerator()
    {
        return RunLocked(() => base.GetEnumerator());
    }
