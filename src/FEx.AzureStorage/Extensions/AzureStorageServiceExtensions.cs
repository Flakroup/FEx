...
{
    public static void Configure(this IAzureStorageService service, string connStr) =>
        service.Configure(connStr, 8);

    public static Task<(CloudBlockBlob blob, bool isSuccess)> CopyBlobAsync(
        this IAzureStorageService service, string containerName, string srcBlob, string destBlob) =>
    ...
    public static Task<bool> DownloadLatestBlobsAsync(
        this IAzureStorageService service, string downloadDir, string containerName, params string[] paths) =>
    ...
    public static Task<bool> ExistsAsync(
        this IAzureStorageService service, string containerName, string path, string fileName) =>
    ...
    public static Task<IList<T>> GetBlobsAsync<T>(
        this IAzureStorageService service, string containerName, string path) where T : CloudBlob =>
        service.GetBlobsAsync<T>(containerName, path, false);

    public static Task<IList<CloudBlockBlob>> GetCloudBlockBlobsAsync(
        this IAzureStorageService service, string containerName, string path) =>
    ...
    public static Task<(FileInfo file, CloudBlockBlobInfo blob)> UploadFileAsync(
        this IAzureStorageService service, string path, bool overwrite, FileInfo file) =>
    ...
    public static Task<IDictionary<FileInfo, CloudBlockBlobInfo>> UploadFilesAsync(
        this IAzureStorageService service, string containerName, string path, params FileInfo[] files) =>
    ...
    public static Task<(string file, CloudBlockBlob blob)> UploadStreamAsync(
        this IAzureStorageService service, string path, bool overwrite, string fileName, Stream stream) =>
