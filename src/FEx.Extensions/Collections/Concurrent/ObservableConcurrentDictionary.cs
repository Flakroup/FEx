...
[Serializable]
public class ObservableConcurrentDictionary<TKey, TValue> : BaseObservableCollection<TValue>, IChangeableCollection, IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue>
{
    ...
    {
        return ContainsKey(key.GetObject<TKey>());
    }
    ...
    {
        return ValueIsEqual(item.Key, item.Value);
    }
    ...
    {
        return _dictionary.ContainsKey(key) && EqualityHelper.IsEqual(ref val, _dictionary[key]);
    }

    private void UpdateWithNotification(TKey key, TValue value)
    {
        (bool hasBeenReplaced, TValue removedValue, TValue newValue) = _dictionary.AddOrReplaceValue(key, () => value);
        OnCollectionChanged(hasBeenReplaced ? NotifyCollectionChangedAction.Replace : NotifyCollectionChangedAction.Add, newValue, removedValue);
    }
}